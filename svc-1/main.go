...
func main() {
	...
	flag.StringVar(&serverPort, "server_port", "8081", "port on which server runs")
	flag.Parse()
	...
	router.Get("/", Get(zipkinClient, generalConfig))
...
func Get(client *zipkinhttp.Client, generalConfig *config.GeneralConfig) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		err := callService2(r.Context(), generalConfig, client)
		if err != nil {
...
func callService2(ctx context.Context, generalConfig *config.GeneralConfig, client *zipkinhttp.Client) error {
